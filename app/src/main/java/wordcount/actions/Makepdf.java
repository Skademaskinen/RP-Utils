            ...
            try(BufferedWriter writer = new BufferedWriter(new FileWriter(new File(String.format(".cache/build/%s.tex", channel.getName()))))){
                ...
                for(Message message : messages.reversed()){
                    ...
                    if(((counter/messages.size())) > percentage){
                        ...
                        if(!test){
                            ((SlashCommandInteractionEvent)event).getHook().editOriginal("```\n"+percentages.keySet().stream().sorted((e1, e2) -> Integer.compare(percentages.get(e1), percentages.get(e2))).map(key -> String.format("%s: %s%%", key.getName(), percentages.get(key))).collect(Collectors.joining("\n"))+"\n```").queue();
                        }
                    }
